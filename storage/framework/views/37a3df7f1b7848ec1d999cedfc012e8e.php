<!-- JavaScript de Bootstrap (CDN) y dependencias -->
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta3/dist/js/bootstrap.bundle.min.js"></script>

<nav class="col-md-3 col-lg-2 d-md-block bg-light sidebar">
    <div class="position-sticky" style="top: 0;">
        <ul class="nav flex-column">
            <li class="nav-item mb-3">
                <a class="nav-link active" aria-current="page" href="<?php echo e(route('home')); ?>">
                    <img src="<?php echo e(asset('images/homecolor.png')); ?>" alt="Inicio" style="width: 50px; height: 50px;" />
                    Inicio
                </a>
            </li>

            <?php if(auth()->user()->hasRole('administrador')): ?>
            <li class="nav-item dropdown mb-3">
                <a class="nav-link dropdown-toggle" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <img src="data:image/png;base64,"
                     alt="Administrador" style="width: 50px; height: 50px;"/> Administrador
                </a>
                <ul class="dropdown-menu">
                    <li>
                        <a class="dropdown-item" href="<?php echo e(route('register')); ?>">
                            <img src="<?php echo e(asset('images/agregarAdmin.png')); ?>" alt="Registrar" style="width: 50px; height: 50px;" />
                            Registrar
                        </a>
                    </li>
                    <li><a class="dropdown-item" href="<?php echo e(route('usuarios.list')); ?>">
                        <img src="<?php echo e(asset('images/listaAdmin.png')); ?>" alt="Lista" style="width: 50px; height: 50px;" />
                        Lista
                    </a></li>
                </ul>
            </li>
            <?php endif; ?>

            <li class="nav-item dropdown mb-3">
                <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <img src="<?php echo e(asset('images/empleados.png')); ?>" alt="Empleados" style="width: 50px; height: 50px;" /> Empleados
                </a>
                <ul class="dropdown-menu">
                    <?php if(auth()->user()->hasRole('administrador') || auth()->user()->hasRole('inocuidad')): ?>
                    <li><a class="dropdown-item" href="<?php echo e(route('empleados.create')); ?>"> <img src="<?php echo e(asset('images/agregarcolor.png')); ?>"
                                alt="Alta" style="width: 50px; height: 50px;" />
                            Alta</a></li>
                    <li><a class="dropdown-item" href="<?php echo e(route('empleados.baja')); ?>"> <img src="<?php echo e(asset('images/bajacolor.png')); ?>"
                                alt="Baja" style="width: 50px; height: 50px;" />
                            Baja</a></li>
                    <li><a class="dropdown-item" href="<?php echo e(route('empleados.reingreso')); ?>"> <img src="<?php echo e(asset('images/volvercolor.png')); ?>"
                                alt="Reingreso" style="width: 50px; height: 50px;" />
                            Reingreso</a></li>
                    <?php endif; ?>
                    <li><a class="dropdown-item" href="<?php echo e(route('empleados.list')); ?>"> <img src="<?php echo e(asset('images/listacolor.png')); ?>"
                                alt="Lista" style="width: 50px; height: 50px;" />
                            Lista</a></li>
                </ul>
            </li>

            <li class="nav-item dropdown mb-3">
                <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <img src="<?php echo e(asset('images/relojcolor.png')); ?>" alt="Horas" style="width: 50px; height: 50px;" /> Horas
                </a>
                <ul class="dropdown-menu">
                    <?php if(auth()->user()->hasRole('administrador') || auth()->user()->hasRole('inocuidad')): ?>
                    <li><a class="dropdown-item" href="<?php echo e(route('horas.horasEntradas')); ?>"> <img src="<?php echo e(asset('images/entradacolor.png')); ?>"
                                alt="Registrar Entradas" style="width: 50px; height: 50px;" />
                            Registrar Entradas</a></li>
                    <li><a class="dropdown-item" href="<?php echo e(route('horas.horasSalidas')); ?>"> <img src="<?php echo e(asset('images/salidascolor.png')); ?>"
                                alt="Registrar Salidas" style="width: 50px; height: 50px;" />
                            Registrar Salidas</a></li>
                    <li><a class="dropdown-item" href="<?php echo e(route('horas.horasExtras')); ?>"> <img src="<?php echo e(asset('images/horasextracolor.png')); ?>"
                                alt="Registrar Horas Extras" style="width: 50px; height: 50px;" />
                            Registrar Horas Extras</a></li>
                    <?php endif; ?>
                    <li><a class="dropdown-item" href="<?php echo e(route('horas.list')); ?>"> <img src="<?php echo e(asset('images/listaasistenciacolor.png')); ?>"
                                alt="Lista de Asistencia" style="width: 50px; height: 50px;" />
                            Lista de Asistencia</a></li>
                </ul>
            </li>

            <li class="nav-item dropdown mb-3">
                <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <img src="<?php echo e(asset('images/vacacionescolor.png')); ?>" alt="Vacaciones" style="width: 50px; height: 50px;" /> Vacaciones
                </a>
                <ul class="dropdown-menu">
                    <?php if(auth()->user()->hasRole('administrador') || auth()->user()->hasRole('inocuidad')): ?>
                    <li><a class="dropdown-item" href="<?php echo e(route('vacaciones.create')); ?>"> <img src="<?php echo e(asset('images/agregarvacacolor.png')); ?>"
                                alt="Registrar" style="width: 50px; height: 50px;" />
                            Registrar</a></li>
                    <?php endif; ?>
                    <li><a class="dropdown-item" href="<?php echo e(route('vacaciones.list')); ?>"> <img src="<?php echo e(asset('images/calendariocolor.png')); ?>"
                                alt="Lista" style="width: 50px; height: 50px;" />
                            Lista</a></li>
                </ul>
            </li>

            <li class="nav-item dropdown mb-3">
                <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <img src="data:image/png;base64,"
                     alt="Incapacidad" style="width: 40px; height: 40px;" /> Incapacidad

                </a>
                <ul class="dropdown-menu">
                    <?php if(auth()->user()->hasRole('administrador') || auth()->user()->hasRole('inocuidad')): ?>
                    <li><a class="dropdown-item" href="<?php echo e(route('incapacidad.create')); ?>"> <img src="data:image/png;base64,"
                                alt="Registrar" style="width: 40px; height: 40px;" />
                            Registrar</a></li>
                    <?php endif; ?>
                    <li><a class="dropdown-item" href="<?php echo e(route('incapacidad.list')); ?>"> <img src="data:image/png;base64,"
                                alt="Lista" style="width: 50px; height: 50px;" />
                            Lista</a></li>
                </ul>
            </li>

            <li class="nav-item dropdown mb-3">
                <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <img src="<?php echo e(asset('images/areascolor.png')); ?>" alt="Áreas" style="width: 50px; height: 50px;" /> Áreas
                </a>
                <ul class="dropdown-menu">
                    <?php if(auth()->user()->hasRole('administrador') || auth()->user()->hasRole('inocuidad')): ?>
                    <li><a class="dropdown-item" href="<?php echo e(route('areas.create')); ?>"> <img src="<?php echo e(asset('images/agregarvacacolor.png')); ?>"
                                alt="Registrar" style="width: 50px; height: 50px;" />
                            Registrar</a></li>
                    <?php endif; ?>
                    <li><a class="dropdown-item" href="<?php echo e(route('areas.list')); ?>"> <img src="<?php echo e(asset('images/listaasistenciacolor.png')); ?>"
                                alt="Lista" style="width: 50px; height: 50px;" />
                            Lista</a></li>
                </ul>
            </li>

        </ul>
    </div>
</nav>
<?php /**PATH C:\xampp\htdocs\Inocuidad_Bima\resources\views/navbar/navbar.blade.php ENDPATH**/ ?>